...
export function TargetingScoring() {
    return (
        <Section className="bg-white">
            <SectionHeader
                title="ZeroFrame™ Targeting & Website Visitor Scoring"
                description="The Audience & Traffic Scoring module distills insights from over 120 trusted data sources to map how your users discover, evaluate, and decide. This intelligence is deployed directly into Meta and TikTok — accelerating campaign performance without relying on slow native targeting models. The result: 30–40% lower CPAs and more intentional, efficient spend."
            />
            ...
            <div className="relative max-w-7xl mx-auto">
                <div className="bg-[#E76F3D] rounded-[2rem] p-8 lg:p-16 relative overflow-hidden">
                    ...
                    <div className="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-24 mb-20 z-10">
                            ...
                            <div className="relative">
                                ...
                                <div className="flex flex-col -space-y-4">
                                    {avatars.map((url, i) => (
                                        ...
                                        >
                                            <Image src={url} alt={`Visitor ${i + 1}`} width={64} height={64} className="object-cover" />
                                        </motion.div>
                                    ))}
                                    <motion.div
                                        initial={{ opacity: 0, x: -20 }}
                        ...
                        <div className="flex flex-col justify-center gap-6">
                            {[
                                { title: "Total conversions", value: "4,040", growth: "+20.6%", sub: "3,349" },
                                { title: "Clickthrough rate", value: "60%", growth: "+21.8%", sub: "49.2%" },
                                { title: "Total revenue", value: "$248,000", growth: "+21.8%", sub: "$203,602" },
                            ].map((item, i) => (
                                <motion.div
                                    key={i}
                                    initial={{ opacity: 0, x: 20 }}
                                ...
                                >
                                    ...
                                    <div className="absolute right-4 bottom-4 w-20 h-10 opacity-30">
                                        <svg viewBox="0 0 100 40" className="w-full h-full stroke-white fill-none" strokeWidth="2">
                                            <path d="M 0,35 Q 20,30 40,35 T 80,10 T 100,5" />
                                        </svg>
                                    </div>

                                    <span className="text-xs text-white/60 font-medium uppercase tracking-wider">{item.title}</span>
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-2xl font-bold">{item.value}</span>
                                        <span className="text-[10px] font-bold text-green-400 bg-green-400/10 px-1.5 py-0.5 rounded-full">{item.growth}</span>
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 pt-12 border-t border-white/10 relative z-10">
                        <div className="space-y-2">
                            <p className="text-white font-bold leading-tight">Audience creation based on real-time behavior, updated daily.</p>
                        </div>
                        <div className="space-y-2">
                            <p className="text-white font-bold leading-tight">Website User Identification & Journey Mapping</p>
                        </div>
                        <div className="space-y-2">
                            <p className="text-white font-bold leading-tight">Intent and readiness signals</p>
                        </div>
                        <div className="space-y-2">
                            <p className="text-white font-bold leading-tight">Messaging alignment by persona</p>
                        </div>
                    </div>
